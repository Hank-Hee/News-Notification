...
> 文档状态：v1.1（与当前 `main` 实现及 2026-07-24 实跑结果一致）
>
...
| 编号 | 功能 | 需求说明 |
|---|---|---|
| FR-01 | 自动触发 | 每天北京时间约 07:30 自动运行，也支持手动运行；工作流文件合入 `main` 时自动执行一次验证 |
...
- 正式生成前通过 Kimi `/models` 预检确认 Secret、国内 API 地址和 `kimi-k2.6` 权限均有效。
...
```mermaid
flowchart TD
    A["定时、手动或工作流变更触发"] --> B["校验 Secret、国内 API 与模型权限"]
...
3. **工作流变更验证**：`.github/workflows/daily-summary.yml` 合入 `main` 时自动运行一次；普通代码或文档 push 不会因此额外消费 Kimi API。
...
6. 调用 Kimi `/models` 做只读预检，确认 Secret 有效、地址可访问且当前 Key 可使用 `kimi-k2.6`。
...
预检只记录 HTTP 状态和脱敏后的错误类型，不打印 API Key。预检通过后，工作流把 `data/config.github.json` 复制成运行时读取的 `data/config.json`。这样仓库可以保留面向 GitHub 的配置模板，同时复用 Horizon 原有的配置加载方式。
...
候选默认每 10 条组成一个批次，当前最多同时执行 1 个分析请求。批量处理可以减少 API 请求数量和重复提示词 Token，同时通过稳定 ID 把结果映射回原条目。单并发是针对国内 Kimi 账户限流和稳定性的保守设置；确认额度与限流允许后才建议提高。
...
4. 使用 `max_completion_tokens`，不使用已弃用的 `max_tokens`；不同阶段分别限制输出规模。

`kimi-k2.6` 的非思考模式采用固定温度。配置文件以 `0.6` 准确描述模型模式，但 Kimi Client 实际不发送 `temperature` 参数，避免固定温度校验导致 400 错误。即使调用方传入温度覆盖值，Kimi K2.6 请求也会忽略该参数。
...
如果模型已经成功返回、但一个大批次的结构化内容无法正确解析，系统会把批次递归拆小，最终尝试逐条分析。单条仍然失败时，该条使用保守默认值并得到 0 分，因此不会越过 7.5 分阈值进入日报。

认证、额度、限流、网络和关键模型参数等 Provider/API 错误不会被误判为 JSON 内容错误，也不会触发递归拆分与请求风暴，而是立即向上抛出。若全部候选都没有得到有效 AI 结果，任务明确失败，不发布“今日暂无重要动态”的误导性空页。只有 AI 分析真实成功、但所有有效分数均低于 7.5 时，才显示正常的空日报说明。
...
2026-07-24 的正式验证运行抓取 461 条、预筛后分析 80 条、筛出 12 条并增强 Top 5；共发出 24 次 AI 请求，输入 57,768 Tokens、输出 33,491 Tokens、合计 91,259 Tokens。该基线用于判断后续请求量或成本是否异常，不代表每天固定消耗。
...
| Temperature | 配置值 0.6；请求中省略 | 符合 K2.6 非思考模式的固定温度要求，避免参数校验错误 |
| Max completion tokens | 512–6144（按阶段） | 批量评分 6144、单条评分/语义去重 2048、概念提取 512、深度分析 4096、趋势 1024 |
| Thinking | disabled | 不进行不必要的深度思考 |
| JSON Mode | enabled | 要求返回有效 JSON 对象 |
| 分析批次 | 10 | 每次最多分析 10 条候选 |
| 分析并发 | 1 | 串行执行评分批次，降低国内接口限流风险 |
| 增强并发 | 1 | 串行处理 Top 5 增强，降低峰值请求压力 |
...
| Kimi 限流、网络或其他 API 错误 | 立即向上抛出，不拆分分析批次 | 否 | 防止把 Provider 故障放大为请求风暴 |
...
| 全部候选分析无有效结果 | 明确报错并终止 | 否 | 不把模型故障伪装成正常的“暂无重要动态” |
